...
// This function takes the min and max values for the left LDR hardcoded at the beginning and maps
them from 0 to 100 int getLeftLDR() {
...
// This function takes the min and max values for the right LDR hardcoded at the beginning and maps
them from 0 to 100 int getRightLDR() {
...
// This function records a set of sensor readings and keeps track of the minimum and maximum values
encountered
...
void lineFollow() {
  while (medFilt(tiltGround) < 9) {
    // Get the position of the line. Note that we *must* provide the "sensors" argument to read_line()
    here, even though we
